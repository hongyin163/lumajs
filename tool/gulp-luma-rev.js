...
var revHash = require('./rev-hash.js');
...
function getManifestFile(opts, cb) {
    ...
    var pth = path.resolve(base, opts.path);
    fs.readFile(pth, 'utf-8', function (err, content) {
        if (err) {
            // not found
            if (err.code === 'ENOENT') {
        ...
        opts.contents = new Buffer(content);
        cb(null, new gutil.File(opts));
    });
}
...
plugin.version = function (pth, opts) {
    if (typeof pth === 'string') {
        pth = { path: pth };
    }

    opts = objectAssign({
        path: 'rev-version.json',
        merge: false,
        // Apply the default JSON transformer.
        // The user can pass in his on transformer if he wants. The only requirement is that it should
        // support 'parse' and 'stringify' methods.
    ...
    return through.obj(function (file, enc, cb) {
        ...
        var version = {
            hash: revHash(file.contents),
            timestamp: gutil.date(new Date(), 'yyyymmddhhMMss')
        }

        this.push(new gutil.File({
            path: opts.path,
            contents: new Buffer(JSON.stringify(version))
        }));
        cb();
    });
}
